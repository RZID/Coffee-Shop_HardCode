        ...
        <form class="d-flex" v-if="getToken" @submit.prevent="">
          <div class="row">
            <div class="col-8">
              ...
              <div class="position-absolute rounded bg-white" v-if="searching">
  ...
  methods: {
    ...
    search () {
      ...
      if (this.searchVal) {
        this.searching = true
        this.isLoading = true
        Axios.get(`${process.env.VUE_APP_BACKEND}/api/products?search=${this.searchVal}`, {
          headers: {
            'token': this.$store.getters['auth/getToken']
          }
        }).then(res => this.searchData = res.data.data).catch(err => console.error(err)).finally(() => {
